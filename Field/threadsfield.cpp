...
CThreadsField::~CThreadsField()
{
  ...
  /***** DELETE MUTEX *****/
  delete(mutex_readInfoWifi);
  delete(mutex_sendInfoRF);
  delete(mutex_readInfoRF);
  delete(mutex_process);
  delete(mutex_wifi_list);
  delete(mutex_RF);
  delete(mutex_field);
  delete(mutex_animal);
  delete(mutex_tcp);
  ...
  /***** DELETE SEMAPHORES *****/
  delete(sem_sendInfoWifi);

  /***** DELETE SIGNALS *****/
  delete(ts_readInfoWifi);
  delete(ts_sendInfoRF);
  delete(ts_readInfoRF);
  delete(ts_process);
}
...
void * CThreadsField::pv_RFComSenderHandler(void *threadid)
{
  ...
  while (1)
  {
    ...
    p_rf->RFComSender(animalAddr,msgRfSender); //send the msgRfSender to animalAddr
...
void  * CThreadsField::pv_RFComReceiverHandler(void *threadid)
{
  ...
  while (1)
  {
    ...
    memset(msgRfReceiver, '\0', 32); //clear the buffer
    ...
    if(!received) { //Message not received
      if(trys < 3) { //trys 3 more times before process next animal
        trys++;
      ...
      else { //if fail 3 times, send to the database the warning,
            //and put the animal in the REDZONE
        trys = 0;
        commandMatch = true;
    ...
    else { //Message Received, prepare the information to send to the DataBase
      trys = 0;
      received = false;
      ...
      if(!commandMatch) // the information receved is wrong
      {
        trys++;
        pthread_mutex_lock(mutex_sendInfoRF);
        pthread_cond_signal(ts_sendInfoRF);
      ...
      else if((int)msgRfReceiver[13]>0 && (int)msgRfReceiver[13]<5 && msgRfReceiver[4]=='I')
      //if is all ok prepare the information to send to the DataBase
      {
    ...
    if((trys == 0) && commandMatch) { // process the next animal
      pthread_mutex_lock(mutex_process);
      pthread_cond_signal(ts_process);
      pthread_mutex_unlock(mutex_process);
    }
...
void  * CThreadsField::pv_WIFIComReceiverHandler(void *threadid)
{
  ...
  while (1)
  {
    ...
    pthread_mutex_unlock(mutex_readInfoWifi);
     ...
     memset(poststring,'\0',300); //clear all the buffers
     memset(post_send,'\0',400);
     memset(aux,'\0',500);
     memset(cReadWifi, '\0', 32);

     mq_wifiReceiver = mq_open(MQ_WIFI_R, O_RDWR  /* |O_CREAT*/);
     if (mq_wifiReceiver == (mqd_t)-1) { //if the queue is OK
       perror("CThreadsField::pv_WIFIComReceiverHandler In mq_open()");
       //exit(1);
      }
      else
      {
        if(p_tcpcom) //if the connection with the database is OK
        {
          //check if all list are empty, (when the system turns ON for the first time)
          pthread_mutex_lock(mutex_field);
          empty=(p_field->checkNULL(GREENZONE) && p_field->checkNULL(YELLOWZONE)
           && p_field->checkNULL(REDZONE));
          pthread_mutex_unlock(mutex_field);

          if(!empty){ //if not empty, get information of the database (field limits)
            //prepare the string to request information
            ...
            // parcing of the response of the database
            ...
            else // if empty, request all animal's information
            {
              snprintf(poststring,300,"field_id=%d",idField);
              snprintf(post_send,500,
              ...
              if(ids>0)
              {
                ...
                {
                  i++;
                  //set all the animals on redlist
                  pthread_mutex_lock(mutex_field);
...
void *CThreadsField::pv_WIFIComSenderHandler(void *threadid)
{
  ...
  while (1)
  {
    ...
    {
      pthread_mutex_lock (mutex_wifi_list);
      p_field->getAnimalInfo(buffer); // get the information to send to database
      pthread_mutex_unlock (mutex_wifi_list);
      ...
      pthread_mutex_lock (mutex_tcp); //send the information
      p_tcpcom->TcpComOpen();
      p_tcpcom->TcpComTransmite(post_send,strlen(post_send));
      ...
      if(aux[333]!='1') // if the field needs to be updated
      {
        pthread_mutex_lock(mutex_animal);
        _ID_ANIMAL=idAnimal;
        pthread_mutex_unlock(mutex_animal);
      ...
      if (new_animal_to_add!=0) // if exist animals to add
      {
        pthread_mutex_lock(mutex_animal);
        _ID_ANIMAL=idAnimal;
        pthread_mutex_unlock(mutex_animal);
...
void *CThreadsField::pv_processAnimalInfoHandler(void *threadid)
{
  ...
  while (1)
  {
    ...
    if (mq_wifiReceiver == (mqd_t)-1) { //queues without erros
    ...
    if(!empty){ //if all list are not empty
      mq_getattr(mq_wifiReceiver, &attr);

      if(attr.mq_curmsgs > 0) { //if have a new configuration to send to the animal
      ...
      pthread_mutex_unlock(mutex_sendInfoRF);
      ...
      else { //if not, send the normal information request
        mq_close(mq_wifiReceiver);

        pthread_mutex_lock(mutex_field); // get an animal to request information
        ...
        else { //If have an animal, send the request mensage to the RF
          pthread_mutex_lock(mutex_RF);
